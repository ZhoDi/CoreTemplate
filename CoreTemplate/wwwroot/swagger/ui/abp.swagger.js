...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    var expireDate = new Date(Date.now() + (responseJSON.Data.ExpireInSeconds * 1000));
                    abp.auth.setToken(responseJSON.Data.AccessToken, expireDate);
                    callback();   
                } else {
                    alert('Login failed !');
                }
        ...
        xhr.open('POST', '/api/Login/Authenticate', true);
        //xhr.setRequestHeader('abp.TenantId', tenantId);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.send("{" + "LoginId:'" + UserName + "'," + "Password:'" + password + "'}");
    };
    ...
    abp.swagger.openAuthDialog = function (loginCallback) {
        ...
        sbpAuthDialog.id = 'abp-auth-dialog';
