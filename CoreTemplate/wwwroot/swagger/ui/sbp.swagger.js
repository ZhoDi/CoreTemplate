...
(function () {
    ...
    sbp.swagger = sbp.swagger || {};

    sbp.swagger.addAuthToken = function () {
        var authToken = sbp.auth.getToken();
        if (!authToken) {
            return false;
        }

        var cookieAuth = new SwaggerClient.ApiKeyAuthorization(sbp.auth.tokenHeaderName, 'Bearer ' + authToken, 'header');
        swaggerUi.api.clientAuthorizations.add('bearerAuth', cookieAuth);
        return true;
    }

    sbp.swagger.addCsrfToken = function () {
        var csrfToken = sbp.security.antiForgery.getToken();
        if (!csrfToken) {
            return false;
        }
        var csrfCookieAuth = new SwaggerClient.ApiKeyAuthorization(sbp.security.antiForgery.tokenHeaderName, csrfToken, 'header');
        swaggerUi.api.clientAuthorizations.add(sbp.security.antiForgery.tokenHeaderName, csrfCookieAuth);
        return true;
    }
    ...
    function loginUserInternal(tenantId, callback) {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    var result = responseJSON.res;
                    var expireDate = new Date(Date.now() + (result.ExpireInSeconds * 1000));
                    sbp.auth.setToken(result.AccessToken, expireDate);
                    callback();   
                } else {
                    alert('Login failed !');
                }
        ...
        //xhr.setRequestHeader('sbp.TenantId', tenantId);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.send("{" + "UserName:'" + UserName + "'," + "Password:'" + password + "'}");
    };

    sbp.swagger.login = function (callback) {
        //Get TenantId first
    ...
    sbp.swagger.logout = function () {
        sbp.auth.clearToken();
    }

    sbp.swagger.closeAuthDialog = function () {
        if (document.getElementById('sbp-auth-dialog')) {
            document.getElementsByClassName("swagger-ui")[1].removeChild(document.getElementById('sbp-auth-dialog'));
        }
    }

    sbp.swagger.openAuthDialog = function (loginCallback) {
        sbp.swagger.closeAuthDialog();

        var sbpAuthDialog = document.createElement('div');
        sbpAuthDialog.className = 'dialog-ux';
        sbpAuthDialog.id = 'sbp-auth-dialog';

        document.getElementsByClassName("swagger-ui")[1].appendChild(sbpAuthDialog);

        // -- backdrop-ux
        var backdropUx = document.createElement('div');
        backdropUx.className = 'backdrop-ux';
        sbpAuthDialog.appendChild(backdropUx);

        // -- modal-ux
        var modalUx = document.createElement('div');
        modalUx.className = 'modal-ux';
        sbpAuthDialog.appendChild(modalUx);
        ...
        closeButton.onclick = sbp.swagger.closeAuthDialog;
